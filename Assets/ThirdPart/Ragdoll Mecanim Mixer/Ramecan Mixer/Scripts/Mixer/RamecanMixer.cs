    ...
    public class RamecanMixer : MonoBehaviour {
        ...
        private void FixedUpdate() {
            foreach (Bone bone in bones) {
                if (!bone.rigidbody.isKinematic) {
                    ...
                    //position drive
                    // if (bone.positionAccuracy > 0 && bone.withAnimationPos) {
                    if (bone.positionAccuracy > 0) {
                ...
                bone.rbPrevPos = bone.rigidbody.position;
                bone.rbPrevRot = bone.rigidbody.rotation;
				// Debug.Log($"{Time.frameCount} FixedUpdate {bone.name} rbPrePos {bone.rbPrevPos} rbVelocity {bone.rigidbody.velocity}");
            }
        }
        ...
        private void Update() {
            foreach (Bone bone in bones) {
                ...
                bone.beforeAnimationLocalPos = bone.animTransform.localPosition;
                bone.beforeAnimationRot = bone.animTransform.rotation;
            }
        }
        ...
        private void Mix() {
            foreach (Bone bone in bones) {
                ...
                bool moveAnchor = bone.beforeAnimationLocalPos != bone.animTransform.localPosition;
                // bone.withAnimationPos = bone.beforeAnimationPos != bone.animTransform.position;
                // bone.withAnimationRot = bone.beforeAnimationRot != bone.animTransform.rotation;

                if (!bone.IsRoot && moveAnchor) {
            ...
            foreach (Bone bone in bones) {
				...
				// Debug.Log($"{Time.frameCount} LateUpdate {bone.name} animationPos {bone.animTransform.position} rigidbosyPos {bone.rigidbody.position} lerpPos {bone.rbLerpPos}");
			}
...
#if UNITY_EDITOR
        private static State[] statesCopy;
        private static int[] bonesParentsCopy;
		public static bool IsStatesCopyNull() => statesCopy == null || statesCopy.Length == 0;
